...
namespace MarchingCubes
{
	 ...
	 {
		  [Tooltip("The max deep and height of the desert dunes, low values")]
		  [Range(0, Constants.MAX_HEIGHT - 1)]
		  public int maxHeightDifference = Constants.MAX_HEIGHT / 5;

		  [Tooltip("Number vertex (y), where the snow end and the rock start")]
		  [Range(0, Constants.MAX_HEIGHT - 1)]
		  public int snowDeep = Constants.MAX_HEIGHT / 5;

		  [Header("Ice columns configuration")]
		  [Tooltip("Scale of the noise used for the ice columns appear")]
		  [Range(0, 100)]
		  public int iceNoiseScale = 40;
		  [Tooltip("Value in the ice noise map where the ice columns appear")]
		  [Range(0, 1)]
		  public float iceApearValue = 0.8f;
		  [Tooltip("Ice columns max height")]
		  [Range(0, Constants.MAX_HEIGHT - 1)]
